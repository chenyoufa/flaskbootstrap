...
$(function () {
    ...
    $('.menuItem').on('click', function menuItem() {
        ...
        $('.menuTab').each(function () {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .WMP_iframe').each(function () {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.WMP_iframe').hide();
                            return false;
                        }
                    });
                }
        ...
        if (addMenuTab) {
            ...
            // 添加选项卡对应的iframe
            var ifrmaeStr = '<iframe class="WMP_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('.mainContent').find('iframe.WMP_iframe').hide().parents('.mainContent').append(ifrmaeStr);
    ...
    $('.tabCloseOther').on('click', function closeOtherTabs() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function () {
            $('.WMP_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    $('.menuTabs').on('click', '.menuTab', function activeTab() {
        ...
        scrollToMenu(this);
        var cs = getQueryStringByName(currentId, "TabChangeRefresh");
        if (cs) {
            refreshTab();
        }
    });
    ...
    function refreshTab() {
        ...
        var target = $('.WMP_iframe[data-id="' + currentId + '"]');
    ...
    $('.tabCloseAll').on('click', function () {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function () {
            $('.WMP_iframe[data-id="' + $(this).data('id') + '"]').show();
